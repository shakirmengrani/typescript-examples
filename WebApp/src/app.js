...
function gethtmlresp(res, view, template) {
...
function gethtmlwithdata(res, view, template) {
    if (template === void 0) { template = false; }
    res.writeHead(config.status.ok, config.ContentType.html);
    res.write(fs.readFileSync(config.View.header + ".html").toString());
    var list = "";
...
function route(req, res) {
    var myUrl = appUrl.parse(req.url);
    var myMethod = req.method;
    var myPath = __dirname + myUrl.pathname;
    var isPathExist = (myUrl.pathname.substring(0, 4) == "/res" ? true : false);
    if (isPathExist) {
...
var server = app.createServer(route);
